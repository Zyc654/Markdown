...
bool xmltoarg(const char *strxmlbuffer) {
    ...
    getxmlbuffer(strxmlbuffer, "host", star.host, 30); // 从xmlbuffer中提取host节点的值。
    if (strlen(star.host) == 0) { logfile.write("host is null.\n"); return false; }
    
    getxmlbuffer(strxmlbuffer, "mode", star.mode, 10); // 从xmlbuffer中提取mode节点的值。
    if(strag.mode != 2)
        starg.mode = 1;
    
    getxmlbuffer(strxmlbuffer, "username", star.username, 30); // 从xmlbuffer中提取username节点的值。
    if(strlen(star.username) == 0) { logfile.write("username is null.\n"); return false; }

    getxmlbuffer(strxmlbuffer, "password", star.password, 30); // 从xmlbuffer中提取password节点的值。
    if(strlen(star.password) == 0) { logfile.write("password is null.\n"); return false; }
    
    getxmlbuffer(strxmlbuffer, "remotepath", star.remotepath, 100); // 从xmlbuffer中提取remotepath节点的值。    
    
    getxmlbuffer(strxmlbuffer, "localpath", star.localpath, 100); // 从xmlbuffer中提取localpath节点的值。
    
    getxmlbuffer(strxmlbuffer, "matchname", star.matchname, 100); // 从xmlbuffer中提取matchname节点的值。   
}