...
map<string, string> mfromok; // 容器一，用于存放已下载成功文件名和文件的修改时间。
list<st_fileinfo> vfromnlist; // 容器二，下载前列出的文件列表。从 nlist 中加载
list<st_fileinfo> vtook; // 容器三 本次不需要西在的文件列表。
list<st_fileinfo> vdownload; // 容器四 本次需要下载的文件列表。
...
bool loadistfile() {
    ...
    while(true) {
        ...
        if(starg.ptype == 1 && starg.checktime == true) { // 如果ptype==1，并且checktime==true，需要获取文件的修改时间。
            if(ftp.mtime(strfilename) == false) { // 调用ftpclient.mtime()方法获取文件的修改时间。
                logfile.write("ftp.mtime(%s) failed.\n", strfilename.c_str()); return false;
            }   
        }
        
        vformnlist.push_back(st_fileinfo(strfilename, "")); // 把文件名和文件的修改时间添加到容器vfilelist中。
